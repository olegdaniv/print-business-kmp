                        ...
                        Row(
                            ...
                            Row(
                                ...
                                layout.previewUrl?.takeIf { it.isNotBlank() }?.let { previewUrl ->
                                    TextButton(onClick = { window.open(previewUrl, "_blank") }) {
                ...
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        editorPreviewUrl.trim().takeIf { it.isNotBlank() }?.let { previewUrl ->
                            OutlinedButton(onClick = { window.open(previewUrl, "_blank") }) {
...
private fun downloadPreview(layoutName: String, previewUrl: String) {
    ...
    val anchor = (document.createElement("a") as HTMLAnchorElement).apply {
        href = normalizedUrl
        if (normalizedUrl.startsWith("data:", ignoreCase = true)) {
            download = fileName
        } else {
            target = "_blank"
...
private fun choosePreviewImage(onSelected: (String) -> Unit) {
    ...
    input.onchange = {
        ...
        if (file != null) {
            ...
            reader.onload = {
                val result = reader.result?.toString()?.trim()
                if (!result.isNullOrEmpty() && result.startsWith("data:", ignoreCase = true)) {
                    onSelected(result)
                }
            }
