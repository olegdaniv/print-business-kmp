...
if ! "${COMPOSE_CMD[@]}" --env-file "$ENV_FILE" -f "$COMPOSE_FILE" pull; then
  fail "Image pull failed. Check registry access and image names in $ENV_FILE."
fi
...
case "${WATCHTOWER_ENABLED,,}" in
  true|1|yes|on)
    ...
    log "Watchtower auto-updates are enabled."
    ;;
  *)
    log "Watchtower auto-updates are disabled."
    ;;
esac

log "Starting Postgres + backend + web..."
